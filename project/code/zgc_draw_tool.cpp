...
static const TinyGlyph3x5 k_tiny_font_3x5[] = {
...
const TinyGlyph3x5 *find_tiny_glyph_3x5(char ch)
{
    if (ch >= 'a' && ch <= 'z')
    {
    ...
    for (const TinyGlyph3x5 &glyph : k_tiny_font_3x5)
    {
        if (glyph.ch == ch)
        {
            return &glyph;
    ...
    return &k_tiny_font_3x5[sizeof(k_tiny_font_3x5) / sizeof(k_tiny_font_3x5[0]) - 1];
}
}

//=====================================================================================================================
// 函数简介     显示单个 3x5 ASCII 小字符
// 备注信息     默认透明背景；字符本体仅 3x5，字符串排版时建议按 4x6 前进。
//=====================================================================================================================
void dbg_char_3x5(uint16 (*img)[image_width], int x, int y, char ch,
                  uint16 color, uint16 bg_color, bool transparent_bg)
{
    const TinyGlyph3x5 *glyph = find_tiny_glyph_3x5(ch);

    for (int row = 0; row < 5; ++row)
    {
        for (int col = 0; col < 3; ++col)
        {
            const bool pixel_on = (glyph->rows[row] & (1 << (2 - col))) != 0;
            if (pixel_on)
            {
                dbg_point(img, x + col, y + row, color);
            }
...
//=====================================================================================================================
// 函数简介     显示 3x5 ASCII 小字符串，支持 '\n' 换行
// 备注信息     每个字符占 4 像素宽，换行高度 6 像素，适合图传参数栏。
//=====================================================================================================================
void dbg_text_3x5(uint16 (*img)[image_width], int x, int y, const char *text,
                  uint16 color, uint16 bg_color, bool transparent_bg)
{
    if (text == nullptr)
    {
    ...
    for (const char *p = text; *p != '\0'; ++p)
    {
        ...
        dbg_char_3x5(img, x, y, *p, color, bg_color, transparent_bg);
        x += 4;
    }
}
